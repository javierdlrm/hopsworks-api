...
from hopsworks.core import dataset_api
from hopsworks.engine import decision_engine_engine


class DecisionEngine:
    def __init__(
        self, name, config_file_path, use_case, job_name=None, id=None, *args, **kwargs
    ):
        self._id = id
        self._name = name
        ...
        if self._configs_dict["use_case"] == "recommendation":
            self._decision_engine_engine = (
                decision_engine_engine.RecommendationDecisionEngineEngine()
            )
        elif self._configs_dict["use_case"] == "search":
            self._decision_engine_engine = (
                decision_engine_engine.SearchDecisionEngineEngine()
            )
    ...
    def from_response_json(cls, json_dict):
        ...
        if "count" not in json_decamelized:
            return DecisionEngine(**json_decamelized)
